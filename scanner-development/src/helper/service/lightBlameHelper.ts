...
import { ExtendedSbomComponentWithDependencies, SbomComponentWithDependencies } from "./blameHelper";
...
class LightBlameHelper {
  ...
  async extendBlameSBOMForSingleApp(libs: ExtendedSbomComponent[], sbomInfo: SbomEvent, app: Application, resourceName: string) {
    ...
    logger.info(`finish handle sbom blame info for repo: ${resourceName}, , events: ${libs.length}`);
  }
        ...
        try {
          ...
          if (blameInfo?.fileName) {
            ...
            if (blameInfo?.startLineNumber && blameInfo?.fileName) {
              if (blameInfo.fromCommitHistory) {
                if (repo.type === "GitLab" || repo.type === "GitHub") {
                  if (singleBlameResponse.link.includes("/commit/")) {
                    ...
                    const baseUrlWithBlob = singleBlameResponse.link.substring(0, indexOfBlob + 6);
                    let baseUrlWithBlobWithCommitSha = baseUrlWithBlob + singleBlameResponse.commitSha + "/" + singleBlameResponse.fileName;
                    if (blameInfo?.startLineNumber >= 0) {
      ...
      for (const securityAlert of securityAlerts) {
        try {
          ...
          if (securityAlert.blame.askedOnce) {
            logger.warn(`not sending for blame: ${securityAlert.securityAlertTypeStr} due askedOnce, resourceName: ${resourceName}`);
            continue;
          }
          if ((securityAlert.fileName === "" || securityAlert.fileName === "NA") && securityAlertType !== SecurityAlertType.sca) {
            logger.warn(`not sending for blame: ${securityAlert.securityAlertTypeStr} fileName is empty, resourceName: ${resourceName}`);
            continue;
          }
          if (securityAlertType === SecurityAlertType.container && securityAlert.securitySubTypeAlertType !== SecurityAlertType.sca) {
            logger.warn(`not sending for blame: ${securityAlert.securityAlertTypeStr} not sca container, resourceName: ${resourceName}`);
            continue;
          }
  ...
  private async sendAndWaitForRes(requests: BlameRequest[], repo: Repo, type: string, index: number, resourceName: string) {
    ...
    try {
      ...
      const url = process.env.LIGHTBLAME_QUEUE_KEY;
  ...
  copyToolResults({ repoName, dir, type, index }: { repoName: string; dir: string; type: string; index: number }): void {
    if (isUploadToS3()) {
      ...
      try {
        ...
        logger.error(`failed to copy tool blameDir result file for repo name ${repoName}`);
      }
    }
  }
  ...
  getCommand(requestPath: string, outputDir: string, repoDir: string, no_git: boolean) {
    ...
    if (repoDir) {
      if (process.env.DEBUG && !process.env.DOCKER_DEBUG) {
        return `python ${process.env.LIGHT_BLAME_PATH} --source ${repoDir} --events-path ${requestPath} --output-dir ${outputDir} ${noGitFlag}`;
      }
      return `python /src/blame_cli.py --source ${repoDir} --events-path ${requestPath} --output-dir ${outputDir} ${noGitFlag}`;
    } else {
      if (process.env.DEBUG && !process.env.DOCKER_DEBUG) {
        return `python ${process.env.LIGHT_BLAME_PATH} --events-path ${requestPath} --output-dir ${outputDir} ${noGitFlag}`;
      }
...
export default LightBlameHelper;
