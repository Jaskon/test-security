...
class policyDeprecatedCommand extends PolicyRulesBase {
  async eval(jsonData) {
    try {
      ...
      const fixLink = "https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions#environment-files";
      const moreInfoLink = "https://github.blog/changelog/2020-10-01-github-actions-deprecating-set-env-and-add-path-commands/";
      const issueName = `Workflow is using unsecured and deprecated commands`;
      const secondaryTitle = `The 'add-path' command has a path injection vulnerability and the 'set-env' command has a potential environment variable injection vulnerability.  In order to use deprecated commands,  'ACTIONS_ALLOW_UNSECURE_COMMANDS' is set to true in the workflow file.`;
      const recommendation = `Don’t use deprecated commands and instead use GitHub Environment Files.`;
...
export default policyDeprecatedCommand;
