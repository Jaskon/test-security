...
class PolicyCICDSecretsRepoVars extends PolicyRulesBase {
  async eval(jsonData) {
    try {
      ...
      const issueName = `Secrets are not being handled securely in your workflow`;
      const secondaryTitle = `Secrets are not being handled securely in your workflow. Secrets are being stored in GitHub Actions as variables in the Variables area but should instead be stored in the Secrets area so they are encrypted.`;
      const recommendation =
        "Please follow GitHub’s best practices for using secrets in your workflows. Move the secret to the Secrets area and don’t store secrets in the Variables area.";
      const learnMoreLink = "https://docs.github.com/en/actions/learn-github-actions/variables#about-variables";
      const fixLink = "https://docs.github.com/en/actions/security-guides/encrypted-secrets";
...
export default PolicyCICDSecretsRepoVars;
